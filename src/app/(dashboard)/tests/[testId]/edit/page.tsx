...
export default function EditTestPage() {
    ...
    if (testId) {
      api.get(`/tests/${testId}`).then(res => {
        if (res.data.data || res.data.status === 'success') {
          ...
          setName(t.name || '');
          setDuration(t.total_time !== undefined && t.total_time !== null ? String(t.total_time) : '');
          ...
          setUnattemptMarks(um !== undefined && um !== null ? String(um) : '0');
          
          const cm = t.correct_marks ?? t.correct_answer_marks;
          setCorrectMarks(cm !== undefined && cm !== null ? String(cm) : '5');
          
          setTotalQuestions(t.total_questions !== undefined && t.total_questions !== null ? String(t.total_questions) : '');
          setTotalMarks(t.total_marks !== undefined && t.total_marks !== null ? String(t.total_marks) : '');
        }
  ...
  useEffect(() => {
    if (initialTest && subjects.length > 0 && !subjectMapped) {
      if (initialTest.subject) {
        const found = subjects.find(s => s.name === initialTest.subject || s.id === initialTest.subject);
        if (found) {
  ...
  useEffect(() => {
    if (initialTest && topics.length > 0 && !topicsMapped) {
      ...
      const topicIds = topicNames.map((name: string) => {
  ...
  useEffect(() => {
    if (selectedTopics.length > 0) {
      api.post('/sub-topics/multi-topics', { topicIds: selectedTopics })
        .then(res => {
        ...
        .catch(() => {
          ...
          if (selectedTopics.length === 1) {
            api.get(`/sub-topics/topic/${selectedTopics[0]}`)
              .then(res => {
                if (res.data.status === 'success' || res.data.data) {
                  setSubTopics(res.data.data || []);
                }
  ...
  useEffect(() => {
    if (initialTest && subTopics.length > 0 && !subTopicsMapped) {
      ...
      const subTopicIds = subTopicNames.map((name: string) => {
        const found = subTopics.find(st => st.name === name || st.id === name);
        return found ? found.id : null;
      }).filter(Boolean) as string[];
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="flex items-center gap-2 text-gray-500">
          <svg className="animate-spin h-5 w-5 text-[#4461F2]" viewBox="0 0 24 24" fill="none">
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-gray-900">Edit Test creation</h1>
        <button onClick={() => router.back()} className="p-2 text-gray-400 hover:text-gray-700 rounded-lg hover:bg-gray-100 transition-colors">
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
          ...
          {(['chapterwise', 'pyq', 'mock'] as const).map(type => (
            <button
              ...
              className={`px-5 py-1.5 rounded-md text-sm font-medium transition-colors ${
                testType === type ? 'bg-white text-[#4461F2] shadow-sm' : 'text-gray-500'
              }`}
            >
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          <div>
            ...
            {subTopics.length === 0 ? (
              <div className="h-12 px-4 border border-gray-200 rounded-lg flex items-center text-sm text-gray-400">
                {selectedTopics.length > 0 ? 'No sub-topics available' : 'Select topics first'}
              </div>
            ) : (
              <div className="border border-gray-200 rounded-lg p-2 min-h-12 flex flex-wrap gap-1.5">
                {subTopics.map(st => (
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Test Difficulty Level</label>
        ...
        <div>
          ...
          <div className="grid grid-cols-5 gap-4">
            {[
              ...
              { label: 'Unattempted', value: unattemptMarks, set: setUnattemptMarks },
              { label: 'Correct Answer', value: correctMarks, set: setCorrectMarks },
              { label: 'No of Questions', value: totalQuestions, set: setTotalQuestions },
        ...
        {error && (
          <div className="p-3 text-sm text-red-600 bg-red-50 rounded-lg border border-red-100">{error}</div>
        )}
